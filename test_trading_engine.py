...
class TestTradingEngine(unittest.TestCase):
    ...
    # --- Anpassung bestehender Tests an neue Architektur ---
    ...
    def test_c3_reduce_only_overshoot_capping(self):
        """FIX C3: reduce_only Fill überschießt -> Partial Fill & Fee adjustiert."""
        fill_long = Fill("o1", 1.0, Side.LONG, 10.0, 100.0, 10.0, 0, False, "BTC")
        self.assertTrue(self.engine.portfolio.process_fill(fill_long))
        
        # 15 vs 10 Position - wird auf 10 gekappt, Fee von 15 auf 10 reduziert
    ...
    def test_c1_cross_symbol_ioc_cancel(self):
        """FIX C1: Cancel-Schleife darf IOCs fremder Symbole nicht killen."""
    ...
    def test_c2_interleaved_multi_symbol_ticks(self):
        """FIX C2: Multi-Symbol Timestamp Processing."""
    ...
    def test_h1_sizing_considers_slippage(self):
        """FIX H1: enter_trade kalkuliert Slippage ein."""
        self.engine.execution.slippage_model = ConstantSlippage(1.0, 5.0) # 5 Slippage
        t1 = Tick(1.0, "BTC", 100.0, 100.0, 1000, 1000)
        self.engine.enter_trade(t1, Side.LONG, stop_distance=10.0, tp_distance=20.0)
        # Entry assumed at 105. Stop distance is 10 from 105 -> 95.
    ...
    def test_h2_marketable_limit_slippage(self):
        """FIX H2 & K3: Marketable Limit zahlt Slippage, gekappt am Preis."""
        ...
        # Ask=100. Slippage=10 -> Target 110, aber capped bei Limit=105
    ...
    def test_h3_maker_taker_fee(self):
        """FIX H3: Limit passive -> Maker, Market/Marketable -> Taker."""
        ...
        # Tick trifft Limit (Preis sinkt auf 90) -> Volume Consume
        self.engine.execution.process_tick(Tick(2.0, "ETH", 90.0, 90.0, 10, 10))
        self.engine.execution.process_tick(Tick(3.0, "ETH", 90.0, 90.0, 10, 10)) # Queue Drain
    ...
    def test_numerics_entry_drift(self):
        """Scale-ins driften nicht wegen float averages."""
        t1 = Tick(1.0, "BTC", 100.0, 100.0, 1000, 1000)
        self.engine.process_ticks([t1])
        # Force 1000 micro fills
        for i in range(1000):
    ...
    def test_m6_sharpe_annualization(self):
        """M6: Resampled Zeit-Bars korrigieren HFT-Sharpe-Verzerrung."""
        curve = [
    ...
    def test_s1_stop_trigger_no_fill_reverts(self):
        """S1: Stop triggert, füllt nicht -> Status = TRIGGERED."""
    ...
    def test_k2_exception_in_callback(self):
        """K2: Callback wirft -> Order auf ERROR, aus Queue gelöscht."""
    ...
    def test_cancel_modify_api(self):
        """Semantik: API für Cancel und Modify."""
    ...
    def test_funding_hook(self):
        """Semantik: Funding reduziert Equity."""
        ...
        self.engine.portfolio.apply_funding(1.0, "BTC", 0.01) # 100 * 0.01 = 1
    ...
    def test_invariants_property(self):
        """Property-Based Invarianten-Check nach jedem künstlichen Tick."""
