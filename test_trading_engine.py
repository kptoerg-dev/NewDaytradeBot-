...
class TestTradingEngine(unittest.TestCase):
    ...
    def test_audit_fix_1_liquidation_blindspot_and_ioc(self):
        """Beweist, dass eine IOC Partial-Fill Order bei Kontocrash korrekte Folgeorders generiert."""
        t1 = Tick(1.0, 100.0, 100.0, 1000, 1000)
        ...
        # Tick 2: Trigger SL. Generiert Exit Order (1000 Qty) am Ende des Ticks.
        # WICHTIG: bid_volume auf 1 gesetzt, damit ein SHORT-Trade nicht voll ausführen kann!
        t2 = Tick(2.0, 89.0, 90.0, 1, 1000) 
        self.engine.process_ticks([t2])
        ...
        # Restliche 999 verfallen wegen IOC. _check_liquidation erkennt den Rest und schickt Order für 999.
        t3 = Tick(3.0, 10.0, 11.0, 1, 1000)
        self.engine.process_ticks([t3])
        ...
        self.assertEqual(self.engine.execution.pending_orders[0].quantity, 999.0) 
    ...
    def test_audit_fix_3_zombie_state_healing(self):
        """Beweist, dass eine SL Order, die vor Execution ungültig wird, abgebrochen wird."""
        t1 = Tick(1.0, 100.0, 100.0, 100, 100)
        self.engine.enter_trade(t1, Side.LONG, stop_distance=5.0, tp_distance=10.0)
        self.engine.process_ticks([t1])
        
        # Tick 2: Trigger -> Submit Order in Warteschlange.
        t2 = Tick(2.0, 94.0, 95.0, 0, 0)
        self.engine.process_ticks([t2])
        
        pos = self.engine.portfolio.positions[self.engine.symbol]
        # Order ist jetzt "auf dem Weg" zur Börse (pending). Status ist CLOSING.
        self.assertEqual(len(self.engine.execution.pending_orders), 1) 
        self.assertEqual(pos.state, PositionState.CLOSING)
        
        # Tick 3: Retracement nach oben. Re-Validierung VOR Execution muss die Order töten.
        t3 = Tick(3.0, 105.0, 106.0, 100, 100)
        self.engine.process_ticks([t3])
        
        # Pending Order wurde gelöscht, Position ist sicher, Status wurde geheilt.
