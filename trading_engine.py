...
class ExecutionEngine:
    ...
    def submit_order(self, order: Order, current_time: float):
        ...
        # Auto-Convert STOP with IOC to GTC since they are resting by definition
    ...
    def get_exposure(self, engine: ExecutionEngine) -> float:
        ...
        for o in engine.pending_orders + engine.active_orders:
            if not o.reduce_only:
                price_est = o.price if o.price else (self.latest_prices[o.symbol].bid if o.symbol in self.latest_prices else 0.0)
...
class BacktestEngine:
    """
    Event-gesteuerte Backtesting-Engine. 
    Neu: `maintenance_margin_rate` steuert die Liquidations-Schwelle (Default: 0.5 = 50% der Notional Initial Margin).
    """
    ...
    def _check_liquidation(self, tick: Tick):
        ...
        maintenance_margin_req = (current_notional / self.max_leverage) * self.maintenance_margin_rate
    ...
    def finalize(self, current_time: float):
        for sym, pos in list(self.portfolio.positions.items()):
            ...
            exit_order = Order(
                ...
                order_type=OrderType.MARKET, time_in_force=TimeInForce.GTC, quantity=pos.quantity, 
        ...
        if self.portfolio.latest_prices:
            for sym, tick in self.portfolio.latest_prices.items():
                synth_tick = Tick(
                    ...
                    bid_volume=1e9, ask_volume=1e9
                )
                fills = self.execution.process_tick(synth_tick, sym)
                for f in fills: self.portfolio.process_fill(f, sym)
                
        self.portfolio.record_equity(current_time)
        assert len(self.portfolio.positions) == 0, f"Es verbleiben offene Positionen: {self.portfolio.positions}"
