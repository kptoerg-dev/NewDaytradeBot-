...
class BacktestEngine:
    ...
    def process_ticks(self, ticks: List[Tick], symbol: str = "BTCUSD"):
        for tick in ticks:
            ...
            # NEU: Re-Validierung VOR der Ausführung (Zombie-State-Fix)
            self._revalidate_pending_exits(tick, symbol)
    ...
    def _revalidate_pending_exits(self, tick: Tick, symbol: str):
        """Löscht anstehende Exit-Orders, wenn die SL/TP-Bedingung durch Retracement entfallen ist."""
        ...
        # Liquidation hat Vorrang. Wenn Equity <= 0, wird nichts gecancelt.
        if not trigger and self.portfolio.equity > 0:
            exit_side = Side.SHORT if pos.side == Side.LONG else Side.LONG
            
            orders_to_remove = [o for o in self.execution.pending_orders if o.symbol == symbol and o.reduce_only and o.side == exit_side]
            for o in orders_to_remove:
                self.execution.pending_orders.remove(o)
                o.status = OrderStatus.CANCELED
    ...
    def _check_sl_tp(self, tick: Tick, symbol: str = "BTCUSD"):
        ...
        # Fallback-Heilung, falls _revalidate nicht gegriffen hat (z.B. IOC Cancel am Tick-Ende)
