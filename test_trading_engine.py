...
class TestTradingEngine(unittest.TestCase):
    ...
    def test_timestamp_regression_rejection(self):
        ...
        t2 = Tick(1.0, 100.0, 100.0, 100, 100)
        self.engine.process_ticks([t1])
        self.engine.process_ticks([t2]) # Same timestamp is now allowed
        
        t3 = Tick(0.5, 100.0, 100.0, 100, 100) # Strict regression
    ...
    def test_critical_sl_tp_transfer_and_trigger(self):
        ...
        # Assertion adjusted for 0-latency engine: The position closes immediately
        self.assertNotIn(self.sym, self.engine.portfolio.positions)
        self.assertEqual(self.engine.portfolio.trade_ledger[-1]["exit_reason"], ExitReason.STOP_LOSS.name)
    ...
    def test_portfolio_leverage_cap_e2e(self):
        self.engine.max_leverage = 2.0 
        ...
        pos = self.engine.portfolio.positions[self.sym]
        self.assertEqual(pos.quantity, 200.0) # Notional = 20k -> fully capped.
        
        self.engine.enter_trade(t1, Side.LONG, stop_distance=5.0, tp_distance=10.0)
        # Position can no longer increase due to hard leverage cap
    ...
    def test_audit_fix_1_liquidation_triggers_properly(self):
        ...
        # Dump price to trigger margin call 
        t2 = Tick(2.0, 92.0, 92.0, 1000, 1000)
        self.engine.process_ticks([t2])
        # Assertion adjusted for 0-latency engine
        self.assertNotIn(self.sym, self.engine.portfolio.positions)
    ...
    def test_stop_order_default_tif_survives(self):
        order = Order("stop1", 1.0, Side.LONG, OrderType.STOP, quantity=10.0, stop_price=105.0)
        self.engine.execution.submit_order(order, 1.0)
        
        t1 = Tick(1.0, 100.0, 100.0, 1000, 1000)
        self.engine.process_ticks([t1])
        # STOP should be auto-converted to GTC and survive without trigger
        self.assertEqual(len(self.engine.execution.active_orders), 1)
        self.assertEqual(self.engine.execution.active_orders[0].time_in_force, TimeInForce.GTC)
        
        t2 = Tick(2.0, 106.0, 106.0, 1000, 1000)
        self.engine.process_ticks([t2])
        self.assertEqual(len(self.engine.execution.active_orders), 0)
        self.assertIn(self.sym, self.engine.portfolio.positions)
        self.assertEqual(self.engine.portfolio.positions[self.sym].quantity, 10.0)
    ...
    def test_performance_analytics(self):
        ...
        self.assertAlmostEqual(stats["Max Drawdown Pct"], 50 / 10100)
