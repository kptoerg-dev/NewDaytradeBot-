...
# Determinismus sichern (Kein uuid4)
...
class OrderStatus(Enum):
    ...
    CANCELED = "CANCELED"
    REJECTED = "REJECTED"
    TRIGGERED = "TRIGGERED"  # FIX S1: Eigener Status für anstehende Stops
    ERROR = "ERROR"          # FIX K2: Bei Callback-Exceptions
...
@dataclass(order=True)
class Tick(Event):
    # FIX C2: symbol ergänzt, Priority-Standardisierung
...
class Order:
    ...
    queue_position: float = 0.0  # Semantik-Fix: Position in der Orderbuch-Warteschlange
...
class Position:
    ...
    side: Side
    quantity: float
    accumulated_notional: float = 0.0  # Numerischer Fix: Verhindert Catastrophic Cancellation
...
class PerformanceAnalytics:
    ...
    def calculate(trade_ledger: List[dict], equity_curve: List[dict], resample_seconds: float = 3600.0) -> dict:
        ...
        # FIX M6: Resampling in Time-Bars für realistische Sharpe/Sortino Ratios
        time_bars = {}
...
class ExecutionEngine:
    ...
    def process_tick(self, tick: Tick, on_fill: Optional[Callable[[Fill], bool]] = None) -> List[Fill]:
        ...
        for o in to_activate:
            ...
            if o.order_type == OrderType.LIMIT and o.price:
                # FIX: LONG (Bid) stellt sich hinter Bids an, SHORT (Ask) hinter Asks.
        ...
        # FIX C1: Symbol-spezifischer IOC Cancel
        ioc_cancellations = []
        for o in self.active_orders.values():
    ...
    def _try_execute(self, order: Order, tick: Tick, avail_bid: float, avail_ask: float) -> Tuple[Optional[Fill], bool]:
        ...
        # FIX H2: Slippage anwenden, aber am Limit deckeln
...
class PortfolioManager:
    ...
    def mark_to_market(self, tick: Tick):
        ...
        for sym, pos in self.positions.items():
            ...
            if not pt: continue
            # Semantik: Nutze geglätteten mark_price statt Bid/Ask Ping-Pong
    ...
    def process_fill(self, fill: Fill) -> bool:
        ...
        # FIX C3: Capping statt Reject bei Reduce-Only Overshoot
        ...
        # Proportionale Reduzierung von accumulated für sauberen Average Price nach Teil-Close
        if pos.quantity > 0:
...
class BacktestEngine:
    ...
    def process_ticks(self, ticks: List[Tick]):
        """Legacy Wrapper für Abwärtskompatibilität. Lädt Events in Queue."""
        for t in ticks:
    ...
    def enter_trade(self, tick: Tick, side: Side, stop_distance: float, tp_distance: float, symbol: str = "BTCUSD"):
        ...
        # FIX H1: Slippage im Entry und Risk-Sizing einbeziehen
